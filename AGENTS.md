...
- **Bar width is fixed** at 30 cells (`BarWidth` const). Dots are spaced (`o o o`) and anchored to absolute even cells; the pacman face is POSITION-driven — small `c` between dots, big `C` on top of a dot (the eat moment, ILoveCandy-style). `barLine()` pads to exactly `width` so the `%` column never shifts. `TestRenderTaskLine_FixedColumns` pins this.
